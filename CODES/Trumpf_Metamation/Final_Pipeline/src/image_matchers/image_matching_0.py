...
def match_image(img_path, top_k=5, cad_library_folder_path=cad_library_folder_path, extensions=extensions):
    ...
    for i in range(len(cad_image_paths)):
        ...
        # calculate the confidence of the current cad image based on the template matching
        curr_confidence = cv2.matchTemplate(img, template, cv2.TM_CCOEFF).max()
    ...
    print("")
    print("Confidence of the top {} matches based on Template Matching:".format(k))
